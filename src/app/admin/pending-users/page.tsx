...
import { ShieldAlert, Monitor, Ban, CheckCircle, ChevronDown, ChevronUp, Trash2, Eye, EyeOff, Save } from 'lucide-react';
...
export default function AdminPendingUsersPage() {
  ...
  if (loading) return <div className="p-8 text-slate-500 font-medium">Loading pending users...</div>;
  ...
  return (
    <>
      <div className="mb-8">
        ...
        <p className="font-medium text-slate-500">View users awaiting verification, suspended accounts, or pending payments.</p>
      </div>
                              ...
                              <div className="overflow-x-auto">
                                <table className="w-full text-left table-fixed">
                                  <thead className="bg-slate-50 dark:bg-dark-3 border-b border-stroke dark:border-dark-3 text-xs font-semibold text-slate-500 uppercase">
                                    <tr>
                                      <th className="px-5 py-3 w-[20%]">IP Address</th>
                                      <th className="px-5 py-3 w-[20%]">Device</th>
                                      <th className="px-5 py-3 w-[20%]">Browser</th>
                                      <th className="px-5 py-3 w-[20%]">Last Seen</th>
                                      <th className="px-5 py-3 w-[10%] text-center">Session Status</th>
                                      <th className="px-5 py-3 w-[10%] text-right">Action</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-stroke dark:divide-dark-3">
                                    {user.sessions.length === 0 ? (
                                      <tr>
                                        <td colSpan={6} className="px-5 py-6 text-center text-xs text-slate-400 font-medium">
                                        ...
                                        return (
                                          <tr key={session.id} className="text-xs hover:bg-slate-50/30 dark:hover:bg-dark-3/10 transition-colors">
                                            ...
                                            <td className="px-5 py-4 text-center">
                                              {session.isActive ? (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-green-50 text-green-600 border border-green-200 dark:bg-green-500/10">
                                                  Active
                                                </span>
                                              ) : (
                                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-500 dark:bg-slate-800">
                                                  Terminated
                                                </span>
                                              )}
                                            </td>
                                            <td className="px-5 py-4 text-right">
                                              {session.isActive && (
                                                <button 
                                                  ...
                                                  className="text-red hover:bg-red/10 px-2.5 py-1.5 rounded-lg border border-red-200 dark:border-red-900/30 transition-all flex items-center justify-center gap-1 ml-auto text-xs font-semibold"
                                                >
